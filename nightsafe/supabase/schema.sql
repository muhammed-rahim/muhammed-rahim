...
create table if not exists public.journeys(
 ...
 start text not null,destination text not null,eta timestamptz not null,contact text,status text not null default 'active' check(status in ('active','arrived')),
 last_lat double precision,last_lon double precision,last_accuracy double precision,last_location_at timestamptz,created_at timestamptz not null default now()
);
...
insert into journeys(start,destination,eta,contact) values(p_start,p_destination,p_eta,p_contact)
...
begin update journeys set last_lat=p_lat,last_lon=p_lon,last_accuracy=p_accuracy,last_location_at=now() where owner_token=p_owner_token and status='active'; return found; end;$$;
create or replace function public.complete_journey(p_owner_token text) returns boolean language plpgsql security definer set search_path=public as $$ begin update journeys set status='arrived' where owner_token=p_owner_token; return found; end;$$;
create or replace function public.get_journey(p_viewer_token text)
returns table(id uuid,start text,destination text,eta timestamptz,contact text,status text,last_lat double precision,last_lon double precision,last_accuracy double precision,last_location_at timestamptz)
language sql security definer set search_path=public as $$ select id,start,destination,eta,contact,status,last_lat,last_lon,last_accuracy,last_location_at from journeys where viewer_token=p_viewer_token limit 1; $$;
